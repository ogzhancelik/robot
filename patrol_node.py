...
def get_specific_model_name(target_x, target_y):
    ...
    # List of valid trash object names (can be teleported)
    TRASH_OBJECTS = ['pringles', 'cocacola', 'coca_cola', 'coke_can', 'coke_can_slim', 
                     'sprite', 'bottle', 'can', 'cup', 'trash']
    ...
    for i, name in enumerate(latest_model_states.name):
        # Skip non-trash objects (robot, ground, walls, etc.)
        name_lower = name.lower()
        is_trash = any(trash in name_lower for trash in TRASH_OBJECTS)
        if not is_trash:
            continue
            
        # Calculate distance to our target point
...
def main():
    ...
    release_client = node.create_client(Trigger, '/release_trash')
                    ...
                    if nav_success:
                        ...
                        if grasp_client.wait_for_service(timeout_sec=2.0):
                            ...
                            if nav.getResult() == TaskResult.SUCCEEDED:
                                print("   At Bin. Releasing trash...")
                                
                                # 4. RELEASE OBJECT (calls /release_trash service)
                                if release_client.wait_for_service(timeout_sec=2.0):
                                    release_req = Trigger.Request()
                                    release_future = release_client.call_async(release_req)
                                    rclpy.spin_until_future_complete(node, release_future)
                                    
                                    release_resp = release_future.result()
                                    if release_resp.success:
                                        print("   >>> SUCCESS: Trash Released!")
                                    else:
                                        print(f"   !!! RELEASE FAILED: {release_resp.message}")
                            else:
